...
for idx in {0..6};do
	medaka_consensus -t 48 -d ../results/"$subject"/contigs.fasta -m ${model[$idx]} -b 50 -o ../results/"$subject"/"$subject"_${name[$idx]} -i ../../basecalling_alignment/results/HG002_R10D0HAC_q10.fastq;
done
